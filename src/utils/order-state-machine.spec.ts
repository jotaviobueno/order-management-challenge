...
describe("OrderStateMachine", () => {
  describe("canTransition", () => {
    it("deve permitir transição de CREATED para ANALYSIS", () => {
      expect(
        OrderStateMachine.canTransition(OrderState.CREATED, OrderState.ANALYSIS)
      ).toBe(true);
    });
    ...
    it("deve impedir transição de ANALYSIS para CREATED", () => {
      expect(
        OrderStateMachine.canTransition(OrderState.ANALYSIS, OrderState.CREATED)
      ).toBe(false);
    });

    it("deve impedir transição para o mesmo estado", () => {
      expect(
        OrderStateMachine.canTransition(OrderState.CREATED, OrderState.CREATED)
      ).toBe(false);
      expect(
        OrderStateMachine.canTransition(
          OrderState.ANALYSIS,
  ...
  describe("getNextState", () => {
    it("deve retornar ANALYSIS como próximo estado de CREATED", () => {
      expect(OrderStateMachine.getNextState(OrderState.CREATED)).toBe(
        OrderState.ANALYSIS
      );
    });

    it("deve retornar COMPLETED como próximo estado de ANALYSIS", () => {
      expect(OrderStateMachine.getNextState(OrderState.ANALYSIS)).toBe(
        OrderState.COMPLETED
      );
    });
  ...
  describe("validateTransition", () => {
    ...
    it("deve lançar BadRequestException para transição inválida de CREATED para COMPLETED", () => {
      ...
      expect(() =>
        ...
        "Transição inválida de CREATED para COMPLETED. Transições permitidas: ANALYSIS"
      );
    });
    ...
    it("deve lançar BadRequestException para transição inválida de COMPLETED", () => {
      ...
      expect(() =>
        ...
        "Transição inválida de COMPLETED para CREATED. Transições permitidas: nenhuma"
      );
    });
    ...
    it("deve lançar BadRequestException para transição para o mesmo estado", () => {
      ...
      expect(() =>
        ...
        "Transição inválida de CREATED para CREATED. Transições permitidas: ANALYSIS"
      );
    });
  });
  ...
  describe("getAllowedTransitions", () => {
    it("deve retornar [ANALYSIS] para estado CREATED", () => {
      const transitions = OrderStateMachine.getAllowedTransitions(
        OrderState.CREATED
      );
  ...
  describe("fluxo completo", () => {
    it("deve permitir fluxo completo do pedido", () => {
      ...
      expect(
        OrderStateMachine.canTransition(currentState, OrderState.ANALYSIS)
      ).toBe(true);
      currentState = OrderStateMachine.advance(currentState);
      expect(currentState).toBe(OrderState.ANALYSIS);

      expect(
        OrderStateMachine.canTransition(currentState, OrderState.COMPLETED)
      ).toBe(true);
      currentState = OrderStateMachine.advance(currentState);
